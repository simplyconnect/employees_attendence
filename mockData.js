/* =========================================================================
   ...
   NOTE ON STATE: everything here lives in memory for the current browser
   session (employee edits, new leave requests, approvals, etc. all work
   and reflect instantly across the UI) but resets on a full page reload,
   since there is no database behind this demo yet. DataService is the
   seam where a real backend would take over persistence.
   ========================================================================= */
...
const MockData = (() => {
  ...
  const LEAVE_ALLOCATIONS = { Annual: 14, Sick: 10, Casual: 8 };

  let employees = null;
  let attendanceByEmp = null;
  let leaveBalances = null;   // empId -> { Annual: {total,used}, Sick: {...}, Casual: {...} }
  ...
  function buildEmployees() {
    ...
    for (let i = 1; i <= 152; i++) {
      ...
      list.push({
        ...
        empCode,
        name: `${first} ${last}`,
        email: `${first}.${last}${i}`.toLowerCase() + "@northbridge.com",
  ...
  return {
    ...
    getPayslip(empId, ym) {
      ...
      const deduction = Math.round(perDay * absent + perDay * 0.5 * halfDays);
      ...
      return {
        ...
        baseSalary: emp.baseSalary, allowances, deduction, gross, net,
        present, absent, halfDays, onLeave, workDays,
      };
    },
