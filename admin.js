...
(async function () {
  ...
  async function drawPayroll(ym) {
    ...
    }).join("") || `<tr><td colspan="8">${emptyRow("No employees match your search.")}</td></tr>`;
    ...
    Utils.openModal(`Payslip — ${s.empName}`, `
      <div class="payslip-card" style="border:none;">
        <div class="payslip-head" style="border-radius:var(--radius-md);">
          <div>
            <div style="font-size:12px;color:#B9C0DA;margin-bottom:4px;">${Utils.MONTHS[+m-1]} ${y}</div>
        ...
        <div class="payslip-body" style="padding:18px 0 0;">
          ...
          <div class="ps-line"><span>Deductions (${s.absent} absent, ${s.halfDays} half-day)</span><b>-${Utils.fmtCurrency(s.deduction)}</b></div>
