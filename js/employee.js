...
(async function () {
  ...
  async function renderLeaveView() {
    ...
    document.getElementById("leaveBalanceRow").innerHTML = Object.entries(bal).map(([type, b]) => {
      const remaining = b.total - b.used;
      const pct = b.total ? b.used / b.total : 0;
      const circumference = 2 * Math.PI * 34;
      const offset = circumference * (1 - pct);
      return `<div class="card leave-bal-card">
        ...
        <div style="font-weight:600;font-size:13.5px;">${type} leave</div>
        <div style="color:var(--muted);font-size:12px;">${b.used} used of ${b.total} days</div>
      </div>`;
    }).join("");
  ...
  document.getElementById("openLeaveBtn").addEventListener("click", () => {
    const overlay = Utils.openModal("Apply for leave", `
      <form id="leaveForm">
    ...
    overlay.querySelector("#lvSubmit").addEventListener("click", async () => {
      ...
      await DataService.submitLeaveRequest({ empId: employee.id, leaveType: document.getElementById("lvType").value, from, to, reason });
      Utils.closeModal();
      Utils.toast("Leave request submitted for approval.");
      renderLeaveView();
    });
  });
  ...
  document.getElementById("openRegularizeBtn").addEventListener("click", () => {
    ...
    overlay.querySelector("#regSubmit").addEventListener("click", async () => {
      ...
      Utils.closeModal();
      Utils.toast("Correction request submitted for approval.");
      renderLeaveView();
    });
  });
    ...
    document.getElementById("payslipContainer").innerHTML = `
      <div class="payslip-card">
        <div class="payslip-head">
          <div>
            <div style="font-size:12px;color:#B9C0DA;margin-bottom:4px;">Payslip for</div>
        ...
        <div class="payslip-body">
          ...
          <div class="ps-line"><span>Absent</span><b>${slip.absent}</b></div>
          ...
          <div class="ps-line"><span>Absence / half-day deduction</span><b>-${Utils.fmtCurrency(slip.deduction)}</b></div>
